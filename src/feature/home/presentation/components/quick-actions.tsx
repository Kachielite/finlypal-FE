...
const QuickActions = ({savingsModal, budgetModal}: {savingsModal: React.RefObject<Modalize>, budgetModal: React.RefObject<Modalize>}) => {
  ...
  const openExpenseModal = (typeOfExpense: string) => {
    useExpenseState.getState().setModalType('add');
    router.push({pathname: '/expense/add-expense', params: {typeOfExpense: typeOfExpense}});
  }
  ...
  return (
    <View className="flex flex-col justify-start items-start w-full gap-y-[18px]">
      ...
      <View className="flex flex-row justify-between w-full">
        <TouchableOpacity onPress={() => openExpenseModal('income')} className="flex items-center gap-y-2.5">
        ...
        <TouchableOpacity onPress={() => openExpenseModal('expense')} className="flex items-center gap-y-2.5">
